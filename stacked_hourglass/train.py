...
def do_training_epoch(train_loader, model, device, optimiser):
    ...
    target = torch.empty((32,20,64,64), device=device, dtype=torch.float32)
    input = torch.empty((32,3,256,256), device=device, dtype=torch.float32)
