...
class Generic(data.Dataset):
    ...
    def __init__(self, dic, is_train=True, inp_res=256, out_res=64, sigma=1,
    ...
    def __getitem__(self, index):
        ...
        pts = torch.Tensor(a[1])
        labels=pts
        # pts[:, 0:2] -= 1  # Convert pts to zero based
        y=[labels[k][1] for k in range(len(labels))]
        x=[labels[k][0] for k in range(len(labels)) if labels[k][0]!=0]
        min_x=np.min(x)
        min_y=np.min(y)
        max_x=np.max(x)
        max_y=np.max(y)
